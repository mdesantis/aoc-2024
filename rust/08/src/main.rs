...
fn antinodes(antennas: &[(i32, i32, char)], rows: i32, cols: i32) -> HashSet<(i32, i32)> {
    antennas
        .iter()
            ...
            antennas
                ...
                .filter_map(move |antenna2| match (antenna1, antenna2) {
                    ((x1, y1, c1), (x2, y2, c2)) if c1 == c2 && (x1, y1) < (x2, y2) => {
                        antennas_pair_to_antinodes(((*x1, *y1), (*x2, *y2)), rows, cols)
                    }
                    _ => None,
                })
        })
...
fn antinodes_amount(input_contents: &str) -> i32 {
    ...
    let in_bound_antinodes = antinodes(&antennas, rows, cols);

    in_bound_antinodes.len() as i32
}

fn main() {
    let result = antinodes_amount(INPUT_CONTENTS);
...
";
    ...
    fn test_in_bound_antinodes_amount() {
        let expected = 14;
        let actual = antinodes_amount(TEST_INPUT_CONTENTS);
